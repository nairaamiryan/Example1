        ...
        <div>
            ...
            <div style={styles.container}>
                ...
                <div style={styles.statsGrid}>
                    ...
                    <StatCard icon={STATISTICS.APPOINTMENTS.ICON} title={STATISTICS.APPOINTMENTS.TITLE} color={STATISTICS.APPOINTMENTS.COLOR} value={counts.reports} />
                ...
                <div style={styles.financesRow}>
                    <div style={styles.financeCard}>
                        ...
                        <div style={{ ...styles.financeValue, color: "#16a34a" }}>+{finances.totalIncome?.toFixed(0)} ֏</div>
                    </div>
                    <div style={styles.financeCard}>
                        <div style={styles.financeIcon}>💸</div>
                        <div style={styles.financeLabel}>Ընդհանուր ծախս</div>
                        <div style={{ ...styles.financeValue, color: "#dc2626" }}>-{finances.totalExpense?.toFixed(0)} ֏</div>
                    </div>
                    <div style={styles.financeCard}>
                        <div style={styles.financeIcon}>📊</div>
                        <div style={styles.financeLabel}>Զուտ հաշվեկշիռ</div>
                        <div style={{ ...styles.financeValue, color: (finances.totalIncome - finances.totalExpense) >= 0 ? "#16a34a" : "#dc2626" }}>
                            {((finances.totalIncome || 0) - (finances.totalExpense || 0)).toFixed(0)} ֏
                        </div>
                    </div>
                </div>
                        ...
                        <div style={styles.chartCard}>
                            ...
                            <ResponsiveContainer width="100%" height={250}>
                                <PieChart>
                                    ...
                                    >
                                        {statusData.map((_, i) => (
                                            <Cell key={i} fill={["#27ae60", "#4a90d9", "#e67e22"][i % 3]} />
                                        ))}
                                    </Pie>
                                    <Tooltip />
                                </PieChart>
                        ...
                        <div style={{ ...styles.chartCard, gridColumn: "1 / -1" }}>
                            ...
                            <ResponsiveContainer width="100%" height={250}>
                                <BarChart data={doctorsData}>
                                    ...
                                    <Tooltip />
                                    <Legend />
                                    <Bar dataKey="count" name="Հիվանդներ" fill="#4a90d9" radius={[6, 6, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                ...
                <div style={styles.tableCard}>
                    <div style={styles.tableHeader}>
                        <h3 style={styles.tableTitle}>🔔 Վերջին ծանուցումները</h3>
                    </div>
                    <table style={styles.table}>
                        <thead>
                            <tr>{["Վերնագիր", "Հաղորդագրություն", "Տեսակ", "Ամսաթիվ"].map((h) => (
...
const styles = {
    ...
    financesRow: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "20px", marginBottom: "32px" },
    financeCard: { background: "white", borderRadius: "12px", padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", textAlign: "center" },
