...
const PaymentAccept = ({ onSave, onClose }) => {
    const [formData, setFormData] = useState({ patientName: "", amount: "", method: "cash", notes: "" });
    const [disabled, setDisabled] = useState(true);

    const handleChange = (e) => {
        const updated = { ...formData, [e.target.name]: e.target.value };
        setFormData(updated);
        setDisabled(!updated.patientName || !updated.amount);
    };
    ...
    return (
        <div style={styles.overlay} onClick={onClose}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <textarea style={styles.textarea} name="notes" placeholder="Նշումներ" value={formData.notes} onChange={handleChange} />
