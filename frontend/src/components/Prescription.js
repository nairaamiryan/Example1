...
const Prescription = ({ onSave, onClose }) => {
    const [formData, setFormData] = useState({ medication: "", dosage: "", duration: "", notes: "" });
    const [disabled, setDisabled] = useState(true);

    const handleChange = (e) => {
        const updated = { ...formData, [e.target.name]: e.target.value };
        setFormData(updated);
        setDisabled(!updated.medication || !updated.dosage || !updated.duration);
    };
        ...
        <div style={styles.overlay} onClick={onClose}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                <div style={styles.modalHeader}>
                    <h2 style={styles.modalTitle}>Բաղադրատոմս դուրս գրել</h2>
                ...
                <input
                    ...
                    type="text"
                    name="duration"
                    placeholder="Տևողություն (օրինակ՝ 7 օր)"
